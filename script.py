...
async def run():
  ...
  for product in PRODUCTS_TO_SEARCH:
    ...
    for item in response.json()["results"]:
      ...
      price = item["price"]
      link = item ["permalink"]
      contains_substring = any(word_to_avoid.lower() in title.lower() for word_to_avoid in product["avoid"])
      
      if not contains_substring and item["accepts_mercadopago"] and item["currency_id"] == "BRL":
        message = f"""
